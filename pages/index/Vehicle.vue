<template>
  <div class="mx-4">
    <h2 class="text-5xl text-center mb-8">Vehicles</h2>
    <b-table :data="list" :loading="isLoading">
      <template slot-scope="props">
        <b-table-column field="title" label="Name">
          <strong>{{ props.row.name }}</strong>
        </b-table-column>
        <b-table-column field="director" label="Vehicle Class">
          {{ props.row.vehicle_class }}
        </b-table-column>
        <b-table-column field="director" label="Length">
          {{ props.row.length }}
        </b-table-column>
        <b-table-column label="Action" centered>
          <span
            class="icon has-text-info cursor-pointer"
            @click="getDetails(props.row.id)"
          >
            <img
              src="data:image/svg+xml;base64,"
            />
          </span>
        </b-table-column>
      </template>
    </b-table>
  </div>
</template>

<script>
export default {
  computed: {
    list() {
      return this.$store.getters.getObject('vehicles')
    },
    isLoading() {
      return this.$store.state.isLoading
    },
  },
  created() {
    this.$store.dispatch('fetch', {
      url: 'https://ghibliapi.herokuapp.com/vehicles',
      key: 'vehicles',
    })
  },
  methods: {
    getDetails(id) {
      this.$store.dispatch('getDetail', {
        url: 'https://ghibliapi.herokuapp.com/vehicles/' + id,
      })
    },
  },
}
</script>
