<template>
  <div class="mx-4">
    <h2 class="text-5xl text-center mb-8">People</h2>
    <b-table :data="list" :loading="isLoading">
      <template slot-scope="props">
        <b-table-column field="title" label="Name">
          <strong>{{ props.row.name }}</strong>
        </b-table-column>
        <b-table-column field="director" label="Gender">
          {{ props.row.gender }}
        </b-table-column>
        <b-table-column field="producer" label="Age">
          {{ props.row.age }}
        </b-table-column>
...
export default {
  computed: {
    list() {
      return this.$store.getters.getObject('people')
    },
    isLoading() {
